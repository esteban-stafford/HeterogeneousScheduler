...
def ppo(workload_file, platform_file, model_path, ac_kwargs=dict(), seed=0, 
    ...
    def update():
        ...
        pi_l_old, n_l_old, v_l_old, entj, entn = sess.run([pi_loss, n_loss, v_loss, approx_ent_j, approx_ent_n], feed_dict=inputs)

        # Training
        for i in range(train_pi_iters):
            _, _, klj, kln = sess.run([train_pi, train_n, approx_kl_j, approx_kl_n], feed_dict=inputs)
            klj = mpi_avg(klj)
        ...
        pi_l_new, n_l_new, v_l_new, klj, kln, cfj, cfn = sess.run([pi_loss, n_loss, v_loss, approx_kl_j, approx_kl_n, clipfrac_j, clipfrac_n], feed_dict=inputs)
